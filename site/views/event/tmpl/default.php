...
<div class="nycchess-item-detail-container">
    ...
    <?php if ($item->image_path) { ?>
        <div class="nycchess-item-detail-image"><img
                src="/<?php echo $item->image_path; ?>" alt="<?php
    ...
    <div class="nycchess-item-detail-primary-location"><?php
        echo htmlentities($item->location->name, ENT_QUOTES);
        ?></div>
    ...
    if (count($this->venues_by_rate['_all'])) { ?>
        <div class="nycchess-item-detail-venues">
            <form name="nycchess-item-detail-register-venue" action="/index.php"
                  method="POST">
                <input type="submit" name="register_venue_submit"
                       value="Register Now!"/>
                <h3>Venues</h3>
                ...
                <table class="nycc-datatable table-venue-list"
                       id="venue-register-list">
                    <thead>
                    <tr>
                        <th>Location</th>
                        <th>Date</th>
                    ...
                    <?php foreach ($this->venues_by_rate['_all'] as $key => $venue) { ?>
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" name="register_venue[]"
                                       class="register-venue-button"
                                       value="<?php echo $venue->id; ?>"
                                       id="register_<?php echo $venue->id; ?>"/><label
                                        for="register_<?php echo $venue->id; ?>"
                                        class="register-venue-label">select</label>
                            </td>
                        </tr>
                    <?php } ?>
                    </tbody>
